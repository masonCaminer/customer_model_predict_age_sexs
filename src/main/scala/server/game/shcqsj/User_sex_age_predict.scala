package server.game.shcqsj

import java.text.SimpleDateFormat

import org.apache.log4j.{Level, Logger}
import org.apache.spark.sql.functions
import server.game.shcqsj.FeatureExport.saveTempInputData
import ModelInputOutput._
import PropertiesFactory.{title, _}
...
object User_sex_age_predict {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val tmp1_result=model_predict_age.transform(trainRawData).select($"ouid" as "uid",$"sex" as "real_sex",$"age" as "real_age",$"label_age"
      ,$"${AgeModelOutColName}" as "model_age",functions.lit(Array("shcqsj")) as "game_label",$"Xdata")
