...
//    sc.textFile("/user/hive/warehouse/data_mining.db/bdl_xy_age_sex_predict_fea/ds=2018-09-27/").filter{x=>
...
//    }.take(5).foreach(println)
  ...
  */
  def filterValidData(dbPath1:String,spark:SparkSession)={
    import spark.implicits._
    val sc=spark.sparkContext
    ...
    validData.saveAsTextFile("/user/hive/warehouse/data_mining.db/bdl_xy_age_sex_predict_fea/ds=xy_valid_train_2018-09-26/")
    val testdata=sc.textFile("/user/hive/warehouse/data_mining.db/bdl_xy_age_sex_predict_fea/ds=xy_valid_train_2018-09-26/")
    println(s"test distinct number : ${testdata.count()}")
//    spark.sql(
//      """
//        |SELECT uid FROM data_mining.bdl_profile_general_gameladel_xy
//        |where ds='2018-10-06'
//        |AND label_age<>model_age
//        |AND label_age<>-1
//      """.stripMargin).createOrReplaceTempView("test2")
//    sc.textFile("/user/hive/warehouse/data_mining.db/bdl_xy_age_sex_predict_fea/ds=2018-10-06").map{rdd=>
//      val data=rdd.split("\001")
//      val ouid=data(0)
//      val ouidArr=data.slice(1,7).mkString("\001")
//      val xdataStr=data.slice(7,111).mkString("\001")
//      (ouid,ouidArr,xdataStr)
//    }.toDF("ouid","ouidArr","xdataStr").createOrReplaceTempView("test1")
//    val tt=spark.sql(
//      """
//        |select A.ouid,A.xdataStr from test1 as A join test2 as B on A.ouid=B.uid
//      """.stripMargin)
//    tt.select("xdataStr").distinct().count()
