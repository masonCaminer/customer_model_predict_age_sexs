...
object FeatureExport {
  ...
  def main(args: Array[String]): Unit = {
    ...
    ExtractFeature(UserportraitCountHiveTable,yesterds, spark = spark)
...
//      """.stripMargin).filter("hr is null ").show()
    val fea = spark.sql(
      s"""
         ...
         |if(max_paytime_interval is null,0,max_paytime_interval) as max_paytime_interval,
         |if(total_paytime_interval is null,0,total_paytime_interval) as total_paytime_interval,
         |if(user_payfrequency  is null,0,CAST(user_payfrequency as INT)) as user_payfrequency,
         ...
         |his_charge,
         |if(user_paytype  is null,0,CAST(user_paytype as INT)) as user_paytype,
         |if(user_payactive  is null,0,CAST(user_payactive as INT)) as user_payactive,
         |CAST(userisrepay as INT) as  userisrepay,
         |CAST(userismotion as INT) as userismotion,
         |if(user_payvariety  is null,0,CAST(user_payvariety as INT)) as user_payvariety
  ...
  def getUser_id_sex_birthday_age(UserTableName:String,ds:String,spark:SparkSession)={
    ...
    spark.sql(
      s"""
        |SELECT id,cast(sex as int) sex, birthday,real_card
        |FROM ${UserTableName}
        |WHERE ds='${ds}'
      """.stripMargin).map{row=>
      val id=row.getAs[Int]("id")
      val sex=if(row.isNullAt(1)) 0 else row.getAs[Int]("sex")
      val birthday=if(row.isNullAt(2)) "" else row.getAs[String]("birthday")
      ...
      val sex_Y=real_sex
        val real_birthday=if(birthday.matches("^[0-9]{8}$") && birthday.slice(0,4)>="1938" && birthday.slice(4,6)<="12" && birthday.slice(6,8)<="31"){
          birthday
        } else if(isTrueIdCard){
