...
import server.platform.xy.PropertiesFactory.title
...
object User_sex_age_predict {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val tmp1_result=model_predict_age.transform(trainRawData).select($"ouid" as "uid",$"sex" as "real_sex",$"age" as "real_age",$"label_age"
      ,$"${AgeModelOutColName}" as "model_age",functions.lit(Array("xyplat")) as "game_label",$"Xdata")
...
//    result.select("model_sex").distinct().show()
