...
//    }.take(5).foreach(println)
    ...
    val trainRawData = sc.textFile(dbPath).filter { x =>
      ...
      map(x => {
        ...
        (ouid, sex, age, birthday, real_card, Xdata, Ydata)
      }).toDF("ouid", "sex", "age", "birthday", "real_card", "Xdata", "Ydata")
    trainRawData
  }
 /*
  *
  ...
  def OutputModelPredictIntoHiveTable(data:DataFrame,curDs:String,
    ...
    spark.sql(
      s"""
        ...
        | select uid,real_sex,real_age,model_sex,model_age,game_label
        | from result
      """.stripMargin)
  }
